...
namespace PUROPORO
{
    ...
    public class GoKartControllerObsolete : NetworkBehaviour
    {
        const int LayerMaskTrack = 1 << 9;
        const int LayerMaskWall = 1 << 10;
        ...
        private void UpdateTriggerPosition( Collider collider, bool firstTrigger )
        {
            ...
            RaycastHit[] array = Physics.RaycastAll( ray, magnitude * 1f, LayerMaskTrack | LayerMaskWall ); // Player | Wall
        ...
        private bool HandleCollision( Vector3 localRayDirection, Vector3 rayOrigin, float rayLength, bool handleCrash, float gndLimit )
        {
            ...
            bool flag = Physics.Raycast( rayOrigin, localRayDirection, out raycastHit, rayLength, LayerMaskTrack | LayerMaskWall ); // Player | Wall
            if( flag ) // "Checkpoint" 태그 확인
            {
        ...
        private Vector3 GetCollisionCheckedPosition(Vector3 fromPosition, Vector3 toPosition)
        {
            ...
            RaycastHit[] array = Physics.RaycastAll(fromPosition, direction, direction.magnitude, LayerMaskTrack | LayerMaskWall ); // Player | AI
        ...
        private bool decideKartContact(float deltaT)
        {
            ...
            for (uint num = 0U; num < 4U; num++)
            {
                ...
                RaycastHit raycastHit;
                m_sus.wheelContact[num] = Physics.Raycast(origin, -m_first.up, out raycastHit, distance, LayerMaskTrack ); // ignore checkpoint layer
